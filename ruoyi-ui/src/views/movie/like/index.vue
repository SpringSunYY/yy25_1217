<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="点赞编号" prop="id">
        <el-input
      ...
      <el-form-item label="名称" prop="movieTitle">
        <el-input
          v-model="queryParams.movieTitle"
          placeholder="请输入名称"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleImport"
          v-hasPermi="['movie:like:import']"
        >导入</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList" :columns="columns"></right-toolbar>
    </el-row>

    <el-table :loading="loading" :data="likeList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="点赞编号" :show-overflow-tooltip="true" v-if="columns[0].visible" prop="id" />
      ...
      <el-table-column label="电影ID" align="center" :show-overflow-tooltip="true" v-if="columns[3].visible" prop="movieId" />
      <el-table-column label="名称" align="center" :show-overflow-tooltip="true" v-if="columns[4].visible" prop="movieTitle" />
      <el-table-column label="封面" align="center" v-if="columns[5].visible" prop="coverUrl" width="100">
        <template slot-scope="scope">
          <image-preview :src="scope.row.coverUrl" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="类型" align="center" :show-overflow-tooltip="true" v-if="columns[6].visible" prop="genres" />
      <el-table-column label="导演" align="center" :show-overflow-tooltip="true" v-if="columns[7].visible" prop="directors" />
      <el-table-column label="国家地区" align="center" :show-overflow-tooltip="true" v-if="columns[8].visible" prop="country" />
      <el-table-column label="主演" align="center" :show-overflow-tooltip="true" v-if="columns[9].visible" prop="actors" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="名称" prop="movieTitle">
          <el-input v-model="form.movieTitle" placeholder="请输入名称" />
        </el-form-item>
      ...
      >
        ...
        <div class="el-upload__tip text-center" slot="tip">
          <div class="el-upload__tip" slot="tip">
            <el-checkbox v-model="upload.updateSupport" /> 是否更新已经存在的用户点赞表数据
          </div>
...
import { listLike, getLike, delLike, addLike, updateLike } from "@/api/movie/like";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/movie/like/importData"
      },
      // 表单校验
      rules: {
        id: [
          { required: true, message: "点赞编号不能为空", trigger: "blur" }
        ],
        userId: [
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (submitData.id != null) {
            updateLike(submitData).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addLike(submitData).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const likeIds = row.id || this.ids;
      this.$modal.confirm('是否确认删除用户点赞表编号为"' + likeIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('movie/like/export', {
        ...this.queryParams
      }, `like_${new Date().getTime()}.xlsx`)
    },
    /** 导入按钮操作 */
    handleImport() {
      this.upload.title = "用户点赞表导入";
      this.upload.open = true;
    },
    /** 下载模板操作 */
    importTemplate() {
      this.download(
        "movie/like/importTemplate",
        {},
        "like_template_" + new Date().getTime() + ".xlsx"
      );
    },
    // 文件上传中处理
