...
from ruoyi_movie.controller import like as like_bp
from ruoyi_movie.domain.entity import Like
from ruoyi_movie.service.like_service import LikeService
...
@PreAuthorize(HasPerm('movie:like:list'))
@JsonSerializer()
...
@PathValidator()
@PreAuthorize(HasPerm('movie:like:query'))
@JsonSerializer()
def get_like(id: int):
    """获取用户点赞表详细信息"""
...
@BodyValidator()
@PreAuthorize(HasPerm('movie:like:edit'))
@Log(title='用户点赞表管理', business_type=BusinessType.UPDATE)
@JsonSerializer()
...
@PathValidator()
@PreAuthorize(HasPerm('movie:like:remove'))
@Log(title='用户点赞表管理', business_type=BusinessType.DELETE)
@JsonSerializer()
...
@PreAuthorize(HasPerm('movie:like:export'))
@Log(title='用户点赞表管理', business_type=BusinessType.EXPORT)
@BaseSerializer()
...
def import_data(
    ...
    file = file[0]
    excel_util = ExcelUtil(Like)
    like_list = excel_util.import_file(file, sheetname="用户点赞表数据")
