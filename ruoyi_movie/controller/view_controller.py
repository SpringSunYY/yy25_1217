...
from ruoyi_movie.controller import view as view_bp
...
@PreAuthorize(HasPerm('movie:view:list'))
@JsonSerializer()
...
@PathValidator()
@PreAuthorize(HasPerm('movie:view:query'))
@JsonSerializer()
def get_view(id: int):
    """获取用户浏览详细信息"""
...
@BodyValidator()
@PreAuthorize(HasPerm('movie:view:add'))
...
@BodyValidator()
@PreAuthorize(HasPerm('movie:view:edit'))
...
@PathValidator()
@PreAuthorize(HasPerm('movie:view:remove'))
...
@PreAuthorize(HasPerm('movie:view:export'))
...
@FileUploadValidator()
@PreAuthorize(HasPerm('movie:view:import'))
